...
n_0_list <- c(5000,10000,20000)  #样本量
#n_0_list2 <- c(500,800,1200,1600,2000,3000)
n_0_list2 <- c(1000,3000,6000)
#n_0_list <- c(1000)  #样本量
n_1_list <- c(400)
gamma_list <- c(0,0.5,1) #gamma大小的调节参数
heter_level_list <- c(0,0.5,1)
#-----
# p_Z_list <- c(3,4,6,8,10,12)
p_Z_list <- c(3,4)
#-----
inter_strength_list <- c(0,0.5,1)
alpha_list <- c(0,0.5,1)
# beta_list <- c(1,2)
                      ...
                      beta_hat_ite <- foreach(ite_k=1:ite_times, .export =c('heter_level')
                                              , .packages = c('fixest','TwoSampleMR','glmnet','mr.raps','penalized','ridge','FNN','data.table'))%dopar%
                        {
                          ...
                          tryCatch({
                            ...
                            p_power_OR <- 2 * pnorm(-abs(omega_hat2 / sd_omega_hat2))
                            p_power_IPW <- 2 * pnorm(-abs(omega_hat3 / sd_omega_hat3))
                            p_power_AIPW <- 2 * pnorm(-abs(omega_hat4 / sd_omega_hat4))
                            p_power_list <- list(p_power_OR,p_power_IPW,p_power_AIPW)
                            
                            p_cover_OR <- 2 * pnorm(-abs((omega_hat2-omega) / sd_omega_hat2))
                            p_cover_IPW <- 2 * pnorm(-abs((omega_hat3-omega) / sd_omega_hat3))
                            p_cover_AIPW <- 2 * pnorm(-abs((omega_hat4-omega) / sd_omega_hat4))
                            p_cover_list <- list(p_cover_OR,p_cover_IPW,p_cover_AIPW)
