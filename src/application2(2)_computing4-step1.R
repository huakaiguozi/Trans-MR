...
data_outcomes_binary <- data_outcomes[,c(8:10)]  #1:3,
...
# COPD  哮喘 排除-哮喘
data_outcome <- data_outcomes_binary[,c(1)]
...
if(dim(data_outcome)[2]== 2){
  data_all <- subset(data_all, `排除-哮喘` == 0)
}
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
C_all <- C_all %>% select(-all_of(c("种族背景",'BMI','COPD', '地理划分')))
# C_all <- C_all %>% select(-all_of(c("种族背景",'BMI','哮喘','排除-哮喘', '地理划分')))

AS_all <- C_all[,c('性别', '年龄')]

X_all <- data_all2$BMI
Y_all <- data_all2$COPD
# Y_all <- data_all2$哮喘
