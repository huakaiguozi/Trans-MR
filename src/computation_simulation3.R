                  ...
                  for (i_beta in 1:len_beta) {
                    for (i_n02 in 1:len_n02) {
                                              ...
                                              , .packages = c('fixest','TwoSampleMR','glmnet','mr.raps','penalized','ridge','FNN','data.table'))%dopar%
                        {
                          ...
                          tryCatch({
                            ...
                            for (i in 1:p_Z) {  
                              ...
                              # inter_term <- X_hat_matrix_OR[,i] * C_0
                              # O_x <- cbind(X_hat_matrix_OR[,i],Z_0[,i],C_0,inter_term)
                              # 
                              # cv_model <- cv.glmnet(O_x, Y_0, alpha = 0,nfolds = 5)
                              # best_lambda <- cv_model$lambda.min
                              # best_model <- glmnet(O_x, Y_0, alpha = 0, lambda = best_lambda,intercept = T)
                              # 
                              
                              model2 <- lm(Y_0~X_hat_matrix_OR[,i] + C_0 + Z_0[,i] + X_hat_matrix_OR[,i] * C_0)  #有Z
                              #model2 <- lm(Y_0~X_hat_matrix_OR[,i] + C_0 + X_hat_matrix_OR[,i] * C_0)  #有Z
                              #inter_term <- X_hat_matrix_OR[,i] * C_0
                              #model2 <- lm(Y_hat_OR_matrix[,i]~X_hat_matrix_OR[,i] + C_0 + inter_term)  #有Z
                              #model2 <- lm(Y_hat_OR_matrix[,i]~X_hat_matrix_OR[,i] + C_0 + Z_0[,i] + X_hat_matrix_OR[,i] * C_0)  #有Z
                              #print(summary(model2))
                              print(cor(X_hat_matrix_OR[,i], vec(X_hat_matrix_OR[,i]) * C_0))
                              print(diag(solve(cor(model.matrix(model2)[, -1]))))
                              print(kappa(model.matrix(model2)[, -1], exact = TRUE))
                              
                              n_coef <- length(model2$coefficients)
                              omega_list2[[i]] <- model2$coefficients[(n_coef-len_omega+1):n_coef]
                              Sigma_list2[[i]] <- vcov(model2)[(n_coef-len_omega+1):n_coef,(n_coef-len_omega+1):n_coef]
                              
                            }
                            
                            W_list2 <- lapply(Sigma_list2, solve)
                            W_total2 <- Reduce(`+`, W_list2)
                            omega_weighted2 <- matrix(0, nrow=p_C, ncol=1)
                            
                            for (i in 1:p_Z) {
                              omega_i <- matrix(omega_list2[[i]], ncol=1)
                              omega_weighted2 <- omega_weighted2 + W_list2[[i]] %*% omega_i
                            }
                            
                            omega_hat2 <- solve(W_total2) %*% omega_weighted2
                            cov_omega2 <- solve(W_total2)
                            #+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                            for (i in 1:p_Z) {
                            ...
                            #---------------------------------------------------------------Trans_IPW0          31
                            for (i in 1:p_Z) {  
                              model3 <- lm(Y_0~X_hat_matrix_IPW[,i] + C_0 + Z_0[,i] + X_hat_matrix_IPW[,i] * C_0)  #有Z
                              
                              print(cor(X_hat_matrix_IPW[,i], vec(X_hat_matrix_IPW[,i]) * C_0))
                              print(diag(solve(cor(model.matrix(model3)[, -1]))))
                              print(kappa(model.matrix(model3)[, -1], exact = TRUE))
                              
                              n_coef <- length(model3$coefficients)
                              omega_list3[[i]] <- model3$coefficients[(n_coef-len_omega+1):n_coef]
                              Sigma_list3[[i]] <- vcov(model3)[(n_coef-len_omega+1):n_coef,(n_coef-len_omega+1):n_coef]
                              
                            }
                            
                            W_list3 <- lapply(Sigma_list3, solve)
                            W_total3 <- Reduce(`+`, W_list3)
                            omega_weighted3 <- matrix(0, nrow=p_C, ncol=1)
                            
                            for (i in 1:p_Z) {
                              omega_i <- matrix(omega_list3[[i]], ncol=1)
                              omega_weighted3 <- omega_weighted3 + W_list3[[i]] %*% omega_i
                            }
                            
                            omega_hat3 <- solve(W_total3) %*% omega_weighted3
                            cov_omega3 <- solve(W_total3)
                            #---------------------------------------------------------------Trans_AIPW0          41
                            for (i in 1:p_Z) {  
                              model4 <- lm(Y_0~X_hat_matrix_AIPW[,i] + C_0 + Z_0[,i] + X_hat_matrix_AIPW[,i] * C_0)  #有Z
                              n_coef <- length(model4$coefficients)
                              omega_list4[[i]] <- model4$coefficients[(n_coef-len_omega+1):n_coef]
                              Sigma_list4[[i]] <- vcov(model4)[(n_coef-len_omega+1):n_coef,(n_coef-len_omega+1):n_coef]
                              
                            }
                            
                            W_list4 <- lapply(Sigma_list4, solve)
                            W_total4 <- Reduce(`+`, W_list4)
                            omega_weighted4 <- matrix(0, nrow=p_C, ncol=1)
                            
                            for (i in 1:3) {
                              omega_i <- matrix(omega_list4[[i]], ncol=1)
                              omega_weighted4 <- omega_weighted4 + W_list4[[i]] %*% omega_i
                            }
                            
                            omega_hat4 <- solve(W_total4) %*% omega_weighted4
                            cov_omega4 <- solve(W_total4)
                            #----------------------------结果保存
                            #beta_hat_vec <- c(beta_hat_1, beta_hat_2, beta_hat_3,beta_hat_4)
                            omega_hat_list <- list(omega_hat2,omega_hat3,omega_hat4)
                            
                            p_power_OR <- 2 * pnorm(-abs(omega_hat2 / sqrt(diag(cov_omega2))))
                            p_power_IPW <- 2 * pnorm(-abs(omega_hat3 / sqrt(diag(cov_omega3))))
                            p_power_AIPW <- 2 * pnorm(-abs(omega_hat4 / sqrt(diag(cov_omega4))))
                            p_power_list <- list(p_power_OR,p_power_IPW,p_power_AIPW)
                            
                            p_cover_OR <- 2 * pnorm(-abs((omega_hat2-omega) / sqrt(diag(cov_omega2))))
                            p_cover_IPW <- 2 * pnorm(-abs((omega_hat3-omega) / sqrt(diag(cov_omega3))))
                            p_cover_AIPW <- 2 * pnorm(-abs((omega_hat4-omega) / sqrt(diag(cov_omega4))))
                      ...
                      for (i in 1:ite_times_actual) {
                        ...
                        # 填充 OR 方法的结果（第 1 行）
                        omega_matrix[start_row, ] <- sim$omega[[1]][, 1]
                        p_power_matrix[start_row, ] <- sim$p[[1]][, 1]
                        p_cover_matrix[start_row, ] <- sim$p_cover[[1]][, 1]
                        
                        # 填充 IPW 方法的结果（第 2 行）
                        omega_matrix[start_row + 1, ] <- sim$omega[[2]][, 1]
                        p_power_matrix[start_row + 1, ] <- sim$p[[2]][, 1]
                        p_cover_matrix[start_row + 1, ] <- sim$p_cover[[2]][, 1]
                        
                        # 填充 AIPW 方法的结果（第 3 行）
                        omega_matrix[start_row + 2, ] <- sim$omega[[3]][, 1]
                        p_power_matrix[start_row + 2, ] <- sim$p[[3]][, 1]
                        p_cover_matrix[start_row + 2, ] <- sim$p_cover[[3]][, 1]
                      }
