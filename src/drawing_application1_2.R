...
pdf(file = paste0(path_output_plots, '/forest_BMI', date_mark, '_', ver, '.pdf'),
    height = 12, width = 25)
grid.newpage()
pushViewport(viewport(layout = grid.layout(1, 3,widths=c(1.67,1,1))))
...
forestplot(
  ...
  clip        = c(-0.06,0.08),#c(Lower2,Upper2),
  is.summary  = is_sum,#is.na(Beta2),#is_sum,
  #fn.ci_nrom  = 'fpDrawDiamondCI',
  hrzl_lines  = list(
    "3" = gpar(lwd=2, col="gray"),
...
png(filename = paste0(path_output_plots, '/forest_BMI', date_mark, '_', ver, '.png'),
