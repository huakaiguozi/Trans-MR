...
Trans_OLS2 <- function(Z_1,C_1,X_1,Z_0,C_0){   ####Trans-OR
  ...
  for(j in 1:dim(Z_0)[2]){
    ...
    # #--------要删
    # pi_hat_all <- rbind(pi_hat_all,pi_hat)
    # alpha_hat_all <- c(alpha_hat_all,alpha_hat)
    # eta_hat_all <- rbind(eta_hat_all,eta_1_hat)
    #--------
    ### 预测0中的X_hat
...
IPW2 <- function(Z_0,Z_1,C_0,C_1,X_1){  #频数估计概率
  ...
  for (i_z in 1:dim(Z_0)[2]) {
    #cat(c('当前位置：',as.character(i_z),'/',as.character(dim(Z_0)[2])))
...
AIPW2 <- function(Z_0,Z_1,C_0,C_1,X_1,C_0_origin,C_1_origin){  #频数估计概率
  ...
  X_hat3_slices <- matrix(nrow = n_0,ncol = dim(Z_0)[2])
  for(j in 1:dim(Z_0)[2]){
    #cat(c('当前位置：',as.character(j),'/',as.character(dim(Z_0)[2])))
    interaction_term_1 <- matrix(rep(NA,n_1*p_C),nrow = n_1)   #改成分p_z维度：z是标量
    for (i in 1:p_C) {
...
ridge_reg <- function(data, indices, lambda) {
  ...
  best_model <- glmnet::glmnet(sample_data[,-1], sample_data[,1], alpha = 0, lambda = lambda,intercept = F)
