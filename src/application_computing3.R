...
path_data_read <- 'data/欧洲人群_zcxys_20W_241112.csv'
...
#
res_0 <- summary_compute(Z=Z_1,X=Y_0,C=AS1)
beta_0 <- res_0$beta
sd_0 <- res_0$sd
p_0 <- res_0$p
eaf_0 <- res_0$eaf
...
if(p_Znm>0){
  ...
  if(p_Zm == 0){
    ...
    write.csv(X_hat_matrix_OR,file=paste(path_output_tables, '/X_OR_',as.character(n_divides),'_',date_mark,'_',ver,'.csv',sep = ''))
    write.csv(X_hat_matrix_IPW,file=paste(path_output_tables, '/X_IPW__',as.character(n_divides),'_',date_mark,'_',ver,'.csv',sep = ''))
    write.csv(X_hat_matrix_AIPW,file=paste(path_output_tables, '/X_AIPW_',as.character(n_divides),'_',date_mark,'_',ver,'.csv',sep = ''))
    #-----------------------------------------
    indice_OR <- c()
    indice_IPW <- c()
    indice_AIPW <- c()
    ...
    if(p_Zm1 >= 1){
      for (i in 1:p_Zm1) {  
        model2 <- lm(Y_0~X_hat_matrix_OR[,i] + C_0_origin + Z_0m1[,i])  #有Z
    ...
    if(p_Zm2>=1){
      ...
      for (i in 1:p_Zm2) {
        ...
        model3 <- lm(Y_0~X_hat_matrix_IPW[,i] + C_0_origin + Z_0m2[,i])  #有Z
    ...
    if(p_Zm3>=1){
      ...
      for (i in 1:p_Zm3) {
        model4 <- lm(Y_0~X_hat_matrix_AIPW[,i] + C_0_origin + Z_0m3[,i])  #有Z
