...
# 设置并行计算的核心数量
cores <- detectCores()
registerDoParallel(cores = cores-24)
#-----------------------------------------------设定
...
#n_0_list <- c(20000)  #样本量
#n_0_list2 <- c(500,800,1200,1600,2000,3000)
n_0_list2 <- c(1000,2000,3000)
n_0_list2 <- c(2000,5000,10000) 
#n_0_list2 <- c(4000,8000,12000)
#n_0_list2 <- c(4000)
#n_0_list <- c(1000)  #样本量
n_1_list <- c(400)
...
gamma_data <- -round(with_seed(1,runif(1000,0.35,0.45)),2)
...
k_gamma <- 1#len_gamma
k_gamma2 <- 1#len_gamma #length(gamma_list)
k_alpha <- len_alpha
k_alpha2 <- len_alpha
k_heter <- len_heter
k_n0 <- len_n0
                ...
                for (i_eta in 1:len_eta) {
                  for (i_beta in 1:len_beta) {
                    for (i_n02 in 1:len_n02) {
                      ...
                      alpha <- alpha_data[1:p_Z_max] * alpha_modifier #/ p_Z_max
                      pi <- pi_data1[1:p_Z_max,1:p_C] * inter_strength #/ p_Z_max
                      pi2 <- pi_data2[1:p_Z_max,1:p_C] * inter_strength #/ p_Z_max
