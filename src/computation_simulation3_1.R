...
pi <- pi_data2[1:p_Z_max,1:p_C] * inter_strength
...
fwrite(dt1,file=paste(path_output_tables, '/mse_pi=1',date_mark,'_',ver,'.csv',sep = ''))
#-----------------------------------------------------------------------
...
zihao2 <- 12 #x轴刻度字号
zihao3 <- 15 #图例文本字号
kedu_up <- ceiling(summary(omega.1)[6]*10)/10
kedu_down <- floor(summary(omega.1)[1]*10)/10
kedu_fen <- 0.1
...
kedu_up2 <- ceiling(summary(omega.2)[6]*10)/10
kedu_down2 <- floor(summary(omega.2)[1]*10)/10
kedu_fen2 <- 0.1
...
ggsave(paste('pi=1_',date_mark,'_',ver,'.png',sep = ''),plot = P3, bg="white", path = path_output_plots,width = 12,height = 9,dpi = 600)
ggsave(paste('pi=1_',date_mark,'_',ver,'.pdf',sep = ''),plot = P3, bg="white", path = path_output_plots,width = 12,height = 9,dpi = 600)
