...
path_data_read <- 'data/50W全数据所有结局数据_250516.csv'
data_k0 <- fread(path_data_read)
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%数据预处理
#--------
data_outcomes <- data_k0[,c(108,109,111,112,c(127:148))]  #250516
#data_outcomes <- data_k0[,c(167,168,170,171,c(186:207))] #250516-2
#data_outcomes <- data_k0[,c(134,135,137,138,c(153:174))] #250516-3

cols_to_remove <- c("HDL","抑郁", "乳腺癌", "排除-乳腺癌", "FVC", "FEV1", "FEV1/FVC", 
                    "PEF", "COPD", "哮喘", "排除-哮喘")

data_outcomes[, (cols_to_remove) := NULL]
...
data_k1 <- data_k0[,c(1:107,110,113:126)]
#data_k1 <- data_k0[,c(1:166,169,172:185)]
#data_k1 <- data_k0[,c(1:133,136,139:152)]
#--------处理缺失
data_outcome <- data_outcomes_continuous[,c(15,4)]
...
data_all_save <- data_all
#-----------------
data_all_m <- data_all[data_all$性别 == 0,] 
data_all_f <- data_all[data_all$性别 == 1,] 
...
#C_all <- C_all %>% select(-all_of(c("种族背景",'BMI','腰围','臀围','FVC','地理划分')))
C_all <- C_all %>% select(-all_of(c("种族背景",'腰围','臀围','地理划分','总胆固醇','甘油三酯','睾酮','LDL-C')))

AS_all <- C_all[,c('年龄')]

X_all <- data_all2$睾酮
Y_all <- data_all2$`LDL-C`
...
basis_cov1 <- data_all2[[indice_basis_name]]
#Norsh为0，South为1
basis_cov2 <- ifelse(basis_cov1 == 'North', 0, 1)  
...
#选一个运行
indice_population_0 <- basis_cov2 == 0
indice_population_1 <- basis_cov2 == 1

indice_population_0 <- basis_cov2 == 1
indice_population_1 <- basis_cov2 == 0
...
Z_all <- Z_all_save[,..indice_e8]

indice_m <- which(rowSums(modifier_matrix) > 0)
Z_all <- Z_all_save[,..indice_m]
#!!!!!!!!!!!!!!!!!!!!!!!
...
results_list <- foreach(k = 1:p_Z, .packages = c("metafor")) %dopar% {
  ...
  for (j in 1:p_C) {
    ...
    if (length(beta_group_vec) > 1) {
      ...
      modifier_row[j] <- as.numeric(p_Q < 0.05)
    } else {
      modifier_p_row[j] <- NA
      modifier_row[j] <- 0
    }
...
sum(colSums(modifier_matrix) <2)
indice_Cnouse <- which(colSums(modifier_matrix) <2)
...
if(p_Znm>0){
  ...
  beta_hat_res <- Twosamle_package5(p_Z=length(indice_notmodify),p_C=p_C,data_0=data3_0,data_1=data3_1)
...
target_population <- rep('north',length(beta_hat_vec))
target_population <- rep('south',length(beta_hat_vec))
