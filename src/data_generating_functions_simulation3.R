...
data_generate_X <- function(n,p_Z,p_Z_max,p_C,param_X){
  ...
  C_1 <- matrix(rbinom(n = p_C_list[1] * n, size = 2, prob = prob_1), nrow = n)
...
################################################################################
data_generate_Y_binary <- function(n,p_Z,p_Z_max,p_C,param_X,param_Y,Intercept){
  #set.seed(seed)
  #U_md <- param_X$U_md
  #---------param_X
  #h <- param_X$heritability
  ...
  X <- Z %*% alpha + C %*% eta_1+ U  + diag_result1 #+ diag_result2
  Y_linear <- Z %*% gamma + X %*% beta + C %*% eta_2 + U + Intercept
  P_Y <- exp(Y_linear)
  Y <- rbinom(n_0,1,P_Y)
...
data_generate_Y <- function(n,p_Z,p_Z_max,p_C,param_X,param_Y){
  ...
  #X <- Z %*% alpha + C %*% eta_1 + diag(Z %*% pi %*% t(C)) + U + epsi_x 
  X <- Z %*% alpha + C %*% eta_1+ U  + diag_result1 #+ diag_result2
  Y <- Z %*% gamma + X %*% beta + C %*% eta_2 + C %*% omega *  X + U
  ...
  param_true <- list(alpha=alpha,pi=pi)
  #---------
  data_list <- list(Z=Z_return,C=C,X=X, Y=Y
                    ,beta=beta,sd=sd,p=p,eaf=eaf,param=param_true,h_c=h_compute,R2=R2,F_stats=F_stats
                    )
  return(data_list)
}
