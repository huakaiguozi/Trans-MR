...
# 全局计数器
i_global <- 0
highlight_rows <- c(2,4,6,8,10,12,14,16)
#highlight_rows <- c(1,3,5,7,9,11,13,15,17)
...
fn <- local({
  ...
  function(..., clr.line, clr.marker) {
    ...
    #   grid::grid.rect(gp = grid::gpar(fill = "#FFE5EC", col = NA))
    # }
    # if (i_global %in% highlight_rows) {
    #   grid::grid.rect(
    #     gp = grid::gpar(
    ...
    # 交替颜色
    clr <- if(i_local == 17){'#FFA630'} else if (i_local %% 2 == 0) "#00B9BF" else "#F9675C"
...
pdf(file = paste0(path_output_plots, '/forest_BMI(1)', date_mark,'-6' ,'_', ver, '.pdf'),
    height = 12, width = 25)
grid.newpage()
...
png(filename = paste0(path_output_plots, '/forest_BMI(1)', date_mark,'-6' ,'_', ver, '.png'),
...
setEPS()
postscript(file = paste0(path_output_plots, '/forest_BMI(1)', date_mark, '-6_', ver, '.eps'),
           height = 12, width = 25, horizontal = FALSE, onefile = FALSE, paper = "special")
grid.newpage()
pushViewport(viewport(layout = grid.layout(1, 3,widths=c(1.8,1,1))))
pushViewport(viewport(layout.pos.col = 1))
#11111
...
tiff(filename = paste0(path_output_plots, '/forest_BMI(1)', date_mark, '-6', '_', ver, '.tiff'),
     height = 12, width = 25, units = "in", res = 500)
grid.newpage()
pushViewport(viewport(layout = grid.layout(1, 3,widths=c(1.8,1,1))))
pushViewport(viewport(layout.pos.col = 1))
