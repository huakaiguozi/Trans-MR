...
Twosamle_package3 <- function(p_Z,p_C,data_0,data_1){
  ...
  data_merge <- harmonise_data(exposure_dat = exp_data,outcome_dat = out_data,action=1) 
  
  
  res_raps <- mr.raps(data_merge,over.dispersion=FALSE)
  beta <- res_raps$beta.hat
  se <- res_raps$beta.se
  #------------
...
Twosamle_package <- function(p_Z,p_C,data_0,data_1,beta){
  n_0 <- dim(data_0$Z)[1]
  n_1 <- dim(data_1$Z)[1]
  ...
  p_cover_vec <- 2 * (1 - pnorm(abs((res_list$beta-beta)/res_list$se)))
...
Trans_OLS2 <- function(Z_1,C_1,X_1,Z_0,C_0){   ####Trans-OR
  ...
  for(j in 1:dim(Z_0)[2]){
    ...
    O_x1 <- cbind(1,Z_1[,j],interaction_term_1)
    ...
    pi_hat <- matrix(Theta_X_1[c((2):length(Theta_X_1))],nrow = 1)
    #--------
    ### 预测0中的X_hat
    diag_result1 <- rowSums((Z_0[,j] %*% pi_hat) * C_0)
    X_hat3 <- Z_0[,j] * alpha_hat + diag_result1 + itc_1
...
IPW3 <- function(Z_0,Z_1,C_0,C_1,X_1,data_type){  #频数估计概率  data_type: 1连续  2分类  3混合
  n_0 <- dim(Z_0)[1]
  n_1 <- dim(Z_1)[1]
  n <- n_0 + n_1
  p_C <- dim(C_0)[2]
  ...
  for (i_z in 1:dim(Z_0)[2]) {
    ...
    p_zcs <- p_z_vec * p_c_vec   #给定条件S=1，就不把S=1也取到前面去了
    # 接下来估计E[X·I(z,c)|S=1]
    V_0 <- cbind(Z_slice_0, C_0)
    V_1 <- cbind(Z_slice_1, C_1)
    ...
    knn_result <- FNN::knn.reg(
      train = V_1,  # V_1 的前三列作为训练数据
      test = V_0,   # V_0 的前三列作为测试数据
      k = 50,               # 找到最近的 50 个邻居
      y = c(X_1)
    )
    
    X_0_estimates <- knn_result$pred
    
    weight <- 1/p_zcs
    X_hat_slice <- weight * X_0_estimates
    X_hat_slices[,i_z] <- X_hat_slice
...
IPW4 <- function(Z_0,Z_1,C_0,C_1,X_1){  #
  n_0 <- dim(Z_0)[1]
  n_1 <- dim(Z_1)[1]
  p_Z <- ncol(Z_0)
  p_C <- ncol(C_0)
  ...
  colnames1 <- c('Z',paste0('C',c(1:p_C)),'X')
  colnames0 <- colnames1[-length(colnames1)]
  
  X_hat_slices <- matrix(nrow = n_0,ncol = p_Z)
  for (i_z in 1:dim(Z_0)[2]) {
    #----------------------先逻辑回归估计概率
    Z_slice_0 <- as.matrix(Z_0[,i_z])
    Z_slice_1 <- as.matrix(Z_1[,i_z])
    
    Z <- rbind(Z_slice_0, Z_slice_1)
    C <- rbind(C_0, C_1)
    S <- c(rep(0,n_0),rep(1,n_1))
    
    logi_model <- glm(S~Z+C, family = "binomial")
    P_S <- predict(logi_model, type = 'response')[1:n_0]
    #-----------------------
    data0 <- cbind(Z_slice_0,C_0)
    data1 <- cbind(Z_slice_1,C_1,X_1)
    colnames(data0) <- colnames0
    colnames(data1) <- colnames1
    data0 <- setDT(as.data.frame(data0))
    data1 <- setDT(as.data.frame(data1))
    data_all <- rbind(data0[,1:(1+p_C)],data1[,1:(1+p_C)])
    
    #data1_avg <- data1[, .(X_avg = mean(X)), by = colnames0]
    data1_sum <- data1[, .(X_sum = sum(X)), by = colnames0]
    data_all_counts <- data.table(data_all)[, .N, by = colnames0]
    # 将出现次数与 data1_sum 结合，按 (Z, C1, C2) 进行合并
    data1_sum <- merge(data1_sum, data_all_counts, by = colnames0, all.x = TRUE)
    data1_sum[, X_avg := X_sum / N]
    
    # 用apply按行拼接data0和data1_avg的列，生成一个字符向量
    data0_combined <- apply(data0[, ..colnames0], 1, paste, collapse = ",")
    data1_combined <- apply(data1_avg[, ..colnames0], 1, paste, collapse = ",")
    
    data0[, X_filled := data1_avg$X_avg[match(data0_combined, data1_combined)]]
    
    # 3. 处理没有匹配到的行，使用最近邻填充
    missing_rows <- data0[is.na(X_filled)]
    
    # 使用 get.knnx 查找最近邻
    #---
    # 先标准化
    data1_norm <- scale(data1[, ..colnames0]) 
    missing_rows_norm <- scale(missing_rows[, ..colnames0])
    knn_result <- FNN::get.knnx(data = data1_norm, 
                                query = missing_rows_norm, 
                                k = 4)
    # #不标准化
    # knn_result <- FNN::get.knnx(data = data1[, ..colnames0], 
    #                             query = missing_rows[, ..colnames0], 
    #                             k = 4)
    #---
    # 对每个缺失行，计算最近邻的 X 均值
    missing_rows[, X_filled := apply(knn_result$nn.index,MARGIN = 1, function(indices) {
      mean(data1$X[indices])  # 计算最近邻的 X 值的均值
    })]
    
    # 4. 将填充了最近邻 X 的数据合并回 data0
    data0[is.na(X_filled), X_filled := missing_rows$X_filled]
    
    X_hat_slices[,i_z] <- data0$X_filled * (1/P_S)
  }
  res_list <- list(X_hat_matrix = X_hat_slices)
  return(res_list)
}

Trans_KNN <- function(Z_0,Z_1,C_0,C_1,X_1){  #最近邻匹配
  #n_0 <- dim(Z_0)[1]
  n_1 <- dim(Z_1)[1]
  #n_k <- floor(log(n_1,3))
  n_k <- floor(sqrt(n_1)/2)
  ...
  for (i_z in 1:dim(Z_0)[2]) {
    ...
    data0 <- cbind(Z_slice_0,C_0)
    data1 <- cbind(Z_slice_1,C_1)
    
    knn_result <- FNN::knn.reg(
      train = data1,  # V_1 的前三列作为训练数据
      test = data0,   # V_0 的前三列作为测试数据
      k = n_k,               # 找到最近的 50 个邻居
      y = c(X_1)
    )
    #X_0_estimates <- knn_result$pred
    X_hat_slices[,i_z] <- knn_result$pred
  }
  res_list <- list(X_hat_matrix = X_hat_slices)
  return(res_list)
}

Trans_KNN2 <- function(Z_0,Z_1,C_0,C_1,X_1){  #最近邻匹配
  n_0 <- dim(Z_0)[1]
  n_1 <- dim(Z_1)[1]
  n_k <- floor(sqrt(n_1))
  ...
  for (i_z in 1:dim(Z_0)[2]) {
    ...
    indice00 <- Z_slice_0 == 0
    indice01 <- Z_slice_0 == 1
    indice02 <- Z_slice_0 == 2
    
    indice10 <- Z_slice_1 == 0
    indice11 <- Z_slice_1 == 1
    indice12 <- Z_slice_1 == 2
    
    # p10 <- sum(indice10)/n_1
    # p11 <- sum(indice11)/n_1
    # p12 <- sum(indice12)/n_1
    # 
    # nk0 <- floor(n_k * p10)
    # nk2 <- floor(n_k * p12)
    # nk1 <- n_k - nk0 - nk2
    ...
    nk0 <- floor(sqrt(n10)/n_divedes)
    nk1 <- floor(sqrt(n11)/n_divedes)
    nk2 <- floor(sqrt(n12)/n_divedes)
    
    data0 <- cbind(Z_slice_0,C_0)
    data1 <- cbind(Z_slice_1,C_1)
    
    data00 <- data0[indice00,]
    data01 <- data0[indice01,]
    data02 <- data0[indice02,]
    
    data10 <- data1[indice10,]
    data11 <- data1[indice11,]
    data12 <- data1[indice12,]
    
    X10 <- X_1[indice10]
    X11 <- X_1[indice11]
    X12 <- X_1[indice12]
    
    knn_result0 <- FNN::knn.reg(
      train = data10,  # V_1 的前三列作为训练数据
      test = data00,   # V_0 的前三列作为测试数据
      k = nk0,               # 找到最近的 50 个邻居
      y = c(X10)
    )
    X00 <- knn_result0$pred
    
    knn_result1 <- FNN::knn.reg(
      train = data11,  # V_1 的前三列作为训练数据
      test = data01,   # V_0 的前三列作为测试数据
      k = nk1,               # 找到最近的 50 个邻居
      y = c(X11)
    )
    X01 <- knn_result1$pred
    
    knn_result2 <- FNN::knn.reg(
      train = data12,  # V_1 的前三列作为训练数据
      test = data02,   # V_0 的前三列作为测试数据
      k = nk2,               # 找到最近的 50 个邻居
      y = c(X12)
    )
    X02 <- knn_result2$pred
    
    X0_slice[indice00] <- X00
    X0_slice[indice01] <- X01
    X0_slice[indice02] <- X02
    
    X_hat_slices[,i_z] <- X0_slice
  }
...
# 示例：自定义岭回归的 Bootstrap 函数
ridge_reg <- function(data, indices, lambda) {
  sample_data <- data[indices, ]
  best_model <- glmnet::glmnet(sample_data[,-1], sample_data[,1], alpha = 0, lambda = lambda,intercept = T)
  return(best_model$beta[1])
}
